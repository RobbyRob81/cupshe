...
#import "CheckoutViewControllerOld.h"
...
@interface CheckoutViewControllerOld ()

@end

@implementation CheckoutViewControllerOld
...
- (void)viewDidLoad
{
    ...
    /*if ([self.config.payment_method isEqualToString:@"Magento"]){
    ...
    } else if ([self.config.payment_method isEqualToString:@"AuthorizeNet"]) {
        [submit addTarget:self action:@selector(authorizenet_buy:) forControlEvents:UIControlEventTouchUpInside];
    } else {
        [submit addTarget:self action:@selector(buy:) forControlEvents:UIControlEventTouchUpInside];
    }*/
...
/*-(IBAction)authorizenet_buy:(id)sender{
...
-(IBAction)authorizenet_buy:(id)sender {
    ...
    c.expirationDate = [NSString stringWithFormat:@"%ld/%ld",(unsigned long)self.config.card.expMonth, (unsigned long)self.config.card.expYear];
    c.cardCode =  self.config.card.cvc;
    ...
    requestType.amount = [totalp stringValue];
    requestType.payment = paymentType;
    //requestType.tax = extendedAmountTypeTax;
    //requestType.shipping = extendedAmountTypeShipping;
    
    CreateTransactionRequest *trequest = [CreateTransactionRequest createTransactionRequest];
    trequest.transactionRequest = requestType;
    //trequest.anetApiRequest.merchantAuthentication.mobileDeviceId = @"358347040811237";
    //trequest.anetApiRequest.merchantAuthentication.sessionToken = _sessionToken;
    trequest.anetApiRequest.merchantAuthentication.transactionKey = @"9BJfp6b6WG325Sqb";
    trequest.anetApiRequest.merchantAuthentication.name = @"5g3KZa9669";
    
    [an purchaseWithRequest:trequest];
