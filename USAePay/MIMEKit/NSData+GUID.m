...
+(NSData *) getNewGUID
{
    ...
    if (guid)
    {
        CFUUIDBytes rawGUID = CFUUIDGetUUIDBytes(guid);
        NSData *dataGUID = [NSData dataWithBytes:&rawGUID length:sizeof(rawGUID)];
        CFRelease(guid);
        return dataGUID;
    }
    else
...
{
    CFUUIDRef guid = CFUUIDCreateFromString(NULL, (CFStringRef)stringGUID);
    if (guid)
    {
        CFUUIDBytes rawGUID = CFUUIDGetUUIDBytes(guid);
        NSData *dataGUID = [NSData dataWithBytes:&rawGUID length:sizeof(rawGUID)];
