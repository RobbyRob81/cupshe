...
+(NSString *) stringWithNewGUID
{
    ...
    if (guid)
    {
        strGUID = CFBridgingRelease(CFUUIDCreateString(NULL, guid));
        CFRelease(guid);
        return strGUID;
    }
    else
...
+(NSString *) stringWithGUID:(NSData *)guidData
{
    ...
    if (guidData == nil)
        return [self stringWithNewGUID];
    
    CFUUIDBytes rawGUID;
    [guidData getBytes:&rawGUID length:sizeof(rawGUID)];
    
    CFUUIDRef guid = CFUUIDCreateFromUUIDBytes(NULL, rawGUID);
    if (guid)
    {
        strGUID = CFBridgingRelease(CFUUIDCreateString(NULL, guid));
        CFRelease(guid);
