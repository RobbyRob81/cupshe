...
@implementation USAePayPriceTierArray

@synthesize items;

+ (USAePayPriceTierArray *)createWithXml:(DDXMLElement *)__node __request:(USAePayRequestResultHandler*) __request
{
...
- (id) initWithXml: (DDXMLElement*) __node __request:(USAePayRequestResultHandler*) __request {
    if(self = [self init])
    {
        ...
        {
            [self.items addObject:(USAePayPriceTier*)[__request createObject:child type:[USAePayPriceTier class]]];
        }
...
{
    for (USAePayPriceTier* item in self.items) {
        DDXMLElement* propertyElement=[__request writeElement:item type:[USAePayPriceTier class] name:@"PriceTier" URI:@"urn:usaepay" parent:__parent skipNullProperty:NO];
        if(propertyElement!=nil)
        {
            [item serialize:propertyElement __request: __request];
        }
    }
    [__request addAttribute:@"type" URI:@"http://www.w3.org/2001/XMLSchema" stringValue:@"xx2:PriceTierArray" element:__parent];
    [__request addAttribute:@"arrayType" URI:@"http://schemas.xmlsoap.org/soap/encoding/" stringValue:@"xx1:PriceTier[]" element:__parent];
...
- (USAePayPriceTier*)objectAtIndex:(NSUInteger)index {
	return [self.items objectAtIndex:index];
}

- (NSArray *)arrayByAddingObject:(USAePayPriceTier*)anObject {
...
- (BOOL)containsObject:(USAePayPriceTier*)anObject {
...
- (NSUInteger)indexOfObject:(USAePayPriceTier*)anObject {
	return [self.items indexOfObject:anObject];
}

- (NSUInteger)indexOfObject:(USAePayPriceTier*)anObject inRange:(NSRange)range {
	return [self.items indexOfObject:anObject inRange:range];
}

- (NSUInteger)indexOfObjectIdenticalTo:(USAePayPriceTier*)anObject {
	return [self.items indexOfObjectIdenticalTo:anObject];
}

- (NSUInteger)indexOfObjectIdenticalTo:(USAePayPriceTier*)anObject inRange:(NSRange)range {
...
- (void)addObject:(USAePayPriceTier*)anObject {
	return [self.items addObject:anObject];
}

- (void)insertObject:(USAePayPriceTier*)anObject atIndex:(NSUInteger)index {
...
- (void)replaceObjectAtIndex:(NSUInteger)index withObject:(USAePayPriceTier*)anObject {
...
- (void)removeObject:(USAePayPriceTier*)anObject inRange:(NSRange)range {
	return [self.items removeObject:anObject inRange:range];
}

- (void)removeObject:(USAePayPriceTier*)anObject {
	return [self.items removeObject:anObject];
}

- (void)removeObjectIdenticalTo:(USAePayPriceTier*)anObject inRange:(NSRange)range {
	return [self.items removeObjectIdenticalTo:anObject inRange:range];
}

- (void)removeObjectIdenticalTo:(USAePayPriceTier*)anObject {
...
+ (id)array {
	return [[ USAePayPriceTierArray alloc] init];
}

+ (id)arrayWithObject:(USAePayPriceTier*)anObject {
	return [[ USAePayPriceTierArray alloc] initWithObjects:anObject, nil];
}

+ (id)arrayWithObjects:(const USAePayPriceTier* *)objects count:(NSUInteger)cnt {
	return [[ USAePayPriceTierArray alloc] initWithObjects:objects count:cnt];
}

+ (id)arrayWithObjects:(USAePayPriceTier*)firstObj, ... {
	USAePayPriceTierArray* a = [ USAePayPriceTierArray array];
	id eachObject;
...
- (id)initWithObjects:(USAePayPriceTier*)firstObj, ... {
	if(self = [self init]) {
		id eachObject;
		va_list argumentList;
		if (firstObj) {
...
+ (id)arrayWithArray:(NSArray *)array {
	return [[ USAePayPriceTierArray alloc] initWithArray:array];
}

+ (id)arrayWithContentsOfFile:(NSString *)path {
	return [[ USAePayPriceTierArray alloc] initWithContentsOfFile:path];
}

+ (id)arrayWithContentsOfURL:(NSURL *)url {
	return [[ USAePayPriceTierArray alloc] initWithContentsOfURL:url];
}

+ (id)arrayWithCapacity:(NSUInteger)numItems {
	return [[ USAePayPriceTierArray alloc] initWithCapacity:numItems];
}

- (id)initWithObjects:(const USAePayPriceTier* *)objects count:(NSUInteger)cnt {
	self = [self init];
	if(self) {
...
- (NSArray *)sortedArrayUsingFunction:(NSInteger (*)(USAePayPriceTier*, USAePayPriceTier*, void *))comparator context:(void *)context {
	return [self.items sortedArrayUsingFunction:comparator context:context];
}

- (NSArray *)sortedArrayUsingFunction:(NSInteger (*)(USAePayPriceTier*, USAePayPriceTier*, void *))comparator context:(void *)context hint:(NSData *)hint {
	return [self.items sortedArrayUsingFunction:comparator context:context hint:hint];
}

- (void)sortUsingFunction:(NSInteger (*)(USAePayPriceTier*, USAePayPriceTier*, void *))compare context:(void *)context {
