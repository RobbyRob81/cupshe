...
@implementation USAePayProductArray

@synthesize items;

+ (USAePayProductArray *)createWithXml:(DDXMLElement *)__node __request:(USAePayRequestResultHandler*) __request
{
...
- (id) initWithXml: (DDXMLElement*) __node __request:(USAePayRequestResultHandler*) __request {
    if(self = [self init])
    {
        ...
        {
            [self.items addObject:(USAePayProduct*)[__request createObject:child type:[USAePayProduct class]]];
        }
...
{
    for (USAePayProduct* item in self.items) {
        DDXMLElement* propertyElement=[__request writeElement:item type:[USAePayProduct class] name:@"Product" URI:@"urn:usaepay" parent:__parent skipNullProperty:NO];
        if(propertyElement!=nil)
        {
            [item serialize:propertyElement __request: __request];
        }
    }
    [__request addAttribute:@"type" URI:@"http://www.w3.org/2001/XMLSchema" stringValue:@"xx2:ProductArray" element:__parent];
    [__request addAttribute:@"arrayType" URI:@"http://schemas.xmlsoap.org/soap/encoding/" stringValue:@"xx1:Product[]" element:__parent];
...
- (USAePayProduct*)objectAtIndex:(NSUInteger)index {
	return [self.items objectAtIndex:index];
}

- (NSArray *)arrayByAddingObject:(USAePayProduct*)anObject {
...
- (BOOL)containsObject:(USAePayProduct*)anObject {
...
- (NSUInteger)indexOfObject:(USAePayProduct*)anObject {
	return [self.items indexOfObject:anObject];
}

- (NSUInteger)indexOfObject:(USAePayProduct*)anObject inRange:(NSRange)range {
	return [self.items indexOfObject:anObject inRange:range];
}

- (NSUInteger)indexOfObjectIdenticalTo:(USAePayProduct*)anObject {
	return [self.items indexOfObjectIdenticalTo:anObject];
}

- (NSUInteger)indexOfObjectIdenticalTo:(USAePayProduct*)anObject inRange:(NSRange)range {
...
- (void)addObject:(USAePayProduct*)anObject {
	return [self.items addObject:anObject];
}

- (void)insertObject:(USAePayProduct*)anObject atIndex:(NSUInteger)index {
...
- (void)replaceObjectAtIndex:(NSUInteger)index withObject:(USAePayProduct*)anObject {
...
- (void)removeObject:(USAePayProduct*)anObject inRange:(NSRange)range {
	return [self.items removeObject:anObject inRange:range];
}

- (void)removeObject:(USAePayProduct*)anObject {
	return [self.items removeObject:anObject];
}

- (void)removeObjectIdenticalTo:(USAePayProduct*)anObject inRange:(NSRange)range {
	return [self.items removeObjectIdenticalTo:anObject inRange:range];
}

- (void)removeObjectIdenticalTo:(USAePayProduct*)anObject {
...
+ (id)array {
	return [[ USAePayProductArray alloc] init];
}

+ (id)arrayWithObject:(USAePayProduct*)anObject {
	return [[ USAePayProductArray alloc] initWithObjects:anObject, nil];
}

+ (id)arrayWithObjects:(const USAePayProduct* *)objects count:(NSUInteger)cnt {
	return [[ USAePayProductArray alloc] initWithObjects:objects count:cnt];
}

+ (id)arrayWithObjects:(USAePayProduct*)firstObj, ... {
	USAePayProductArray* a = [ USAePayProductArray array];
	id eachObject;
...
- (id)initWithObjects:(USAePayProduct*)firstObj, ... {
	if(self = [self init]) {
		id eachObject;
		va_list argumentList;
		if (firstObj) {
...
+ (id)arrayWithArray:(NSArray *)array {
	return [[ USAePayProductArray alloc] initWithArray:array];
}

+ (id)arrayWithContentsOfFile:(NSString *)path {
	return [[ USAePayProductArray alloc] initWithContentsOfFile:path];
}

+ (id)arrayWithContentsOfURL:(NSURL *)url {
	return [[ USAePayProductArray alloc] initWithContentsOfURL:url];
}

+ (id)arrayWithCapacity:(NSUInteger)numItems {
	return [[ USAePayProductArray alloc] initWithCapacity:numItems];
}

- (id)initWithObjects:(const USAePayProduct* *)objects count:(NSUInteger)cnt {
	self = [self init];
	if(self) {
...
- (NSArray *)sortedArrayUsingFunction:(NSInteger (*)(USAePayProduct*, USAePayProduct*, void *))comparator context:(void *)context {
	return [self.items sortedArrayUsingFunction:comparator context:context];
}

- (NSArray *)sortedArrayUsingFunction:(NSInteger (*)(USAePayProduct*, USAePayProduct*, void *))comparator context:(void *)context hint:(NSData *)hint {
	return [self.items sortedArrayUsingFunction:comparator context:context hint:hint];
}

- (void)sortUsingFunction:(NSInteger (*)(USAePayProduct*, USAePayProduct*, void *))compare context:(void *)context {
