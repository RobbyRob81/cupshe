...
@implementation USAePayTransactionObjectArray
...
- (id) initWithXml: (DDXMLElement*) __node __request:(USAePayRequestResultHandler*) __request {
    if(self = [self init])
    {
        ...
        {
            [self.items addObject:(USAePayTransactionObject*)[__request createObject:child type:[USAePayTransactionObject class]]];
        }
...
{
    for (USAePayTransactionObject* item in self.items) {
        DDXMLElement* propertyElement=[__request writeElement:item type:[USAePayTransactionObject class] name:@"TransactionObject" URI:@"urn:usaepay" parent:__parent skipNullProperty:NO];
        if(propertyElement!=nil)
        {
            [item serialize:propertyElement __request: __request];
        }
    }
    [__request addAttribute:@"type" URI:@"http://www.w3.org/2001/XMLSchema" stringValue:@"xx2:TransactionObjectArray" element:__parent];
    [__request addAttribute:@"arrayType" URI:@"http://schemas.xmlsoap.org/soap/encoding/" stringValue:@"xx1:TransactionObject[]" element:__parent];
...
- (void)removeObjectIdenticalTo:(USAePayTransactionObject*)anObject inRange:(NSRange)range {
	return [self.items removeObjectIdenticalTo:anObject inRange:range];
}

- (void)removeObjectIdenticalTo:(USAePayTransactionObject*)anObject {
...
+ (id)array {
	return [[ USAePayTransactionObjectArray alloc] init];
}

+ (id)arrayWithObject:(USAePayTransactionObject*)anObject {
	return [[ USAePayTransactionObjectArray alloc] initWithObjects:anObject, nil];
}

+ (id)arrayWithObjects:(const USAePayTransactionObject* *)objects count:(NSUInteger)cnt {
	return [[ USAePayTransactionObjectArray alloc] initWithObjects:objects count:cnt];
}

+ (id)arrayWithObjects:(USAePayTransactionObject*)firstObj, ... {
	USAePayTransactionObjectArray* a = [ USAePayTransactionObjectArray array];
	id eachObject;
...
- (id)initWithObjects:(USAePayTransactionObject*)firstObj, ... {
	if(self = [self init]) {
		id eachObject;
		va_list argumentList;
		if (firstObj) {
...
+ (id)arrayWithArray:(NSArray *)array {
	return [[ USAePayTransactionObjectArray alloc] initWithArray:array];
}

+ (id)arrayWithContentsOfFile:(NSString *)path {
	return [[ USAePayTransactionObjectArray alloc] initWithContentsOfFile:path];
}

+ (id)arrayWithContentsOfURL:(NSURL *)url {
	return [[ USAePayTransactionObjectArray alloc] initWithContentsOfURL:url];
}

+ (id)arrayWithCapacity:(NSUInteger)numItems {
	return [[ USAePayTransactionObjectArray alloc] initWithCapacity:numItems];
}
