...
@implementation USAePayBatchStatusArray
...
- (id) initWithXml: (DDXMLElement*) __node __request:(USAePayRequestResultHandler*) __request {
    if(self = [self init])
    {
        ...
        {
            [self.items addObject:(USAePayBatchStatus*)[__request createObject:child type:[USAePayBatchStatus class]]];
        }
...
{
    for (USAePayBatchStatus* item in self.items) {
        DDXMLElement* propertyElement=[__request writeElement:item type:[USAePayBatchStatus class] name:@"BatchStatus" URI:@"urn:usaepay" parent:__parent skipNullProperty:NO];
        if(propertyElement!=nil)
        {
            [item serialize:propertyElement __request: __request];
        }
    }
    [__request addAttribute:@"type" URI:@"http://www.w3.org/2001/XMLSchema" stringValue:@"xx2:BatchStatusArray" element:__parent];
    [__request addAttribute:@"arrayType" URI:@"http://schemas.xmlsoap.org/soap/encoding/" stringValue:@"xx1:BatchStatus[]" element:__parent];
...
- (void)removeObjectIdenticalTo:(USAePayBatchStatus*)anObject inRange:(NSRange)range {
	return [self.items removeObjectIdenticalTo:anObject inRange:range];
}

- (void)removeObjectIdenticalTo:(USAePayBatchStatus*)anObject {
...
+ (id)array {
	return [[ USAePayBatchStatusArray alloc] init];
}

+ (id)arrayWithObject:(USAePayBatchStatus*)anObject {
	return [[ USAePayBatchStatusArray alloc] initWithObjects:anObject, nil];
}

+ (id)arrayWithObjects:(const USAePayBatchStatus* *)objects count:(NSUInteger)cnt {
	return [[ USAePayBatchStatusArray alloc] initWithObjects:objects count:cnt];
}

+ (id)arrayWithObjects:(USAePayBatchStatus*)firstObj, ... {
	USAePayBatchStatusArray* a = [ USAePayBatchStatusArray array];
	id eachObject;
...
- (id)initWithObjects:(USAePayBatchStatus*)firstObj, ... {
	if(self = [self init]) {
		id eachObject;
		va_list argumentList;
		if (firstObj) {
...
+ (id)arrayWithArray:(NSArray *)array {
	return [[ USAePayBatchStatusArray alloc] initWithArray:array];
}

+ (id)arrayWithContentsOfFile:(NSString *)path {
	return [[ USAePayBatchStatusArray alloc] initWithContentsOfFile:path];
}

+ (id)arrayWithContentsOfURL:(NSURL *)url {
	return [[ USAePayBatchStatusArray alloc] initWithContentsOfURL:url];
}

+ (id)arrayWithCapacity:(NSUInteger)numItems {
	return [[ USAePayBatchStatusArray alloc] initWithCapacity:numItems];
}
