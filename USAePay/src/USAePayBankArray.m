...
@implementation USAePayBankArray

@synthesize items;

+ (USAePayBankArray *)createWithXml:(DDXMLElement *)__node __request:(USAePayRequestResultHandler*) __request
{
...
- (id) initWithXml: (DDXMLElement*) __node __request:(USAePayRequestResultHandler*) __request {
    if(self = [self init])
    {
        ...
        {
            [self.items addObject:(USAePayBank*)[__request createObject:child type:[USAePayBank class]]];
        }
...
{
    for (USAePayBank* item in self.items) {
        DDXMLElement* propertyElement=[__request writeElement:item type:[USAePayBank class] name:@"Bank" URI:@"urn:usaepay" parent:__parent skipNullProperty:NO];
        if(propertyElement!=nil)
        {
            [item serialize:propertyElement __request: __request];
        }
    }
    [__request addAttribute:@"type" URI:@"http://www.w3.org/2001/XMLSchema" stringValue:@"xx2:BankArray" element:__parent];
    [__request addAttribute:@"arrayType" URI:@"http://schemas.xmlsoap.org/soap/encoding/" stringValue:@"xx1:Bank[]" element:__parent];
...
- (USAePayBank*)objectAtIndex:(NSUInteger)index {
	return [self.items objectAtIndex:index];
}

- (NSArray *)arrayByAddingObject:(USAePayBank*)anObject {
...
- (BOOL)containsObject:(USAePayBank*)anObject {
...
- (NSUInteger)indexOfObject:(USAePayBank*)anObject {
	return [self.items indexOfObject:anObject];
}

- (NSUInteger)indexOfObject:(USAePayBank*)anObject inRange:(NSRange)range {
	return [self.items indexOfObject:anObject inRange:range];
}

- (NSUInteger)indexOfObjectIdenticalTo:(USAePayBank*)anObject {
	return [self.items indexOfObjectIdenticalTo:anObject];
}

- (NSUInteger)indexOfObjectIdenticalTo:(USAePayBank*)anObject inRange:(NSRange)range {
...
- (void)addObject:(USAePayBank*)anObject {
	return [self.items addObject:anObject];
}

- (void)insertObject:(USAePayBank*)anObject atIndex:(NSUInteger)index {
...
- (void)replaceObjectAtIndex:(NSUInteger)index withObject:(USAePayBank*)anObject {
...
- (void)removeObject:(USAePayBank*)anObject inRange:(NSRange)range {
	return [self.items removeObject:anObject inRange:range];
}

- (void)removeObject:(USAePayBank*)anObject {
	return [self.items removeObject:anObject];
}

- (void)removeObjectIdenticalTo:(USAePayBank*)anObject inRange:(NSRange)range {
	return [self.items removeObjectIdenticalTo:anObject inRange:range];
}

- (void)removeObjectIdenticalTo:(USAePayBank*)anObject {
...
+ (id)array {
	return [[ USAePayBankArray alloc] init];
}

+ (id)arrayWithObject:(USAePayBank*)anObject {
	return [[ USAePayBankArray alloc] initWithObjects:anObject, nil];
}

+ (id)arrayWithObjects:(const USAePayBank* *)objects count:(NSUInteger)cnt {
	return [[ USAePayBankArray alloc] initWithObjects:objects count:cnt];
}

+ (id)arrayWithObjects:(USAePayBank*)firstObj, ... {
	USAePayBankArray* a = [ USAePayBankArray array];
	id eachObject;
...
- (id)initWithObjects:(USAePayBank*)firstObj, ... {
	if(self = [self init]) {
		id eachObject;
		va_list argumentList;
		if (firstObj) {
...
+ (id)arrayWithArray:(NSArray *)array {
	return [[ USAePayBankArray alloc] initWithArray:array];
}

+ (id)arrayWithContentsOfFile:(NSString *)path {
	return [[ USAePayBankArray alloc] initWithContentsOfFile:path];
}

+ (id)arrayWithContentsOfURL:(NSURL *)url {
	return [[ USAePayBankArray alloc] initWithContentsOfURL:url];
}

+ (id)arrayWithCapacity:(NSUInteger)numItems {
	return [[ USAePayBankArray alloc] initWithCapacity:numItems];
}

- (id)initWithObjects:(const USAePayBank* *)objects count:(NSUInteger)cnt {
	self = [self init];
	if(self) {
...
- (NSArray *)sortedArrayUsingFunction:(NSInteger (*)(USAePayBank*, USAePayBank*, void *))comparator context:(void *)context {
	return [self.items sortedArrayUsingFunction:comparator context:context];
}

- (NSArray *)sortedArrayUsingFunction:(NSInteger (*)(USAePayBank*, USAePayBank*, void *))comparator context:(void *)context hint:(NSData *)hint {
	return [self.items sortedArrayUsingFunction:comparator context:context hint:hint];
}

- (void)sortUsingFunction:(NSInteger (*)(USAePayBank*, USAePayBank*, void *))compare context:(void *)context {
