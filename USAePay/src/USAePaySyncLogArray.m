...
@implementation USAePaySyncLogArray
...
- (id) initWithXml: (DDXMLElement*) __node __request:(USAePayRequestResultHandler*) __request {
    if(self = [self init])
    {
        ...
        {
            [self.items addObject:(USAePaySyncLog*)[__request createObject:child type:[USAePaySyncLog class]]];
        }
...
{
    for (USAePaySyncLog* item in self.items) {
        DDXMLElement* propertyElement=[__request writeElement:item type:[USAePaySyncLog class] name:@"SyncLog" URI:@"urn:usaepay" parent:__parent skipNullProperty:NO];
        if(propertyElement!=nil)
        {
            [item serialize:propertyElement __request: __request];
        }
    }
    [__request addAttribute:@"type" URI:@"http://www.w3.org/2001/XMLSchema" stringValue:@"xx2:SyncLogArray" element:__parent];
    [__request addAttribute:@"arrayType" URI:@"http://schemas.xmlsoap.org/soap/encoding/" stringValue:@"xx1:SyncLog[]" element:__parent];
...
- (void)removeObjectIdenticalTo:(USAePaySyncLog*)anObject inRange:(NSRange)range {
	return [self.items removeObjectIdenticalTo:anObject inRange:range];
}

- (void)removeObjectIdenticalTo:(USAePaySyncLog*)anObject {
...
+ (id)array {
	return [[ USAePaySyncLogArray alloc] init];
}

+ (id)arrayWithObject:(USAePaySyncLog*)anObject {
	return [[ USAePaySyncLogArray alloc] initWithObjects:anObject, nil];
}

+ (id)arrayWithObjects:(const USAePaySyncLog* *)objects count:(NSUInteger)cnt {
	return [[ USAePaySyncLogArray alloc] initWithObjects:objects count:cnt];
}

+ (id)arrayWithObjects:(USAePaySyncLog*)firstObj, ... {
	USAePaySyncLogArray* a = [ USAePaySyncLogArray array];
	id eachObject;
...
- (id)initWithObjects:(USAePaySyncLog*)firstObj, ... {
	if(self = [self init]) {
		id eachObject;
		va_list argumentList;
		if (firstObj) {
...
+ (id)arrayWithArray:(NSArray *)array {
	return [[ USAePaySyncLogArray alloc] initWithArray:array];
}

+ (id)arrayWithContentsOfFile:(NSString *)path {
	return [[ USAePaySyncLogArray alloc] initWithContentsOfFile:path];
}

+ (id)arrayWithContentsOfURL:(NSURL *)url {
	return [[ USAePaySyncLogArray alloc] initWithContentsOfURL:url];
}

+ (id)arrayWithCapacity:(NSUInteger)numItems {
	return [[ USAePaySyncLogArray alloc] initWithCapacity:numItems];
}
