...
@implementation USAePaySearchParamArray

@synthesize items;

+ (USAePaySearchParamArray *)createWithXml:(DDXMLElement *)__node __request:(USAePayRequestResultHandler*) __request
{
...
- (id) initWithXml: (DDXMLElement*) __node __request:(USAePayRequestResultHandler*) __request {
    if(self = [self init])
    {
        ...
        {
            [self.items addObject:(USAePaySearchParam*)[__request createObject:child type:[USAePaySearchParam class]]];
        }
...
{
    for (USAePaySearchParam* item in self.items) {
        DDXMLElement* propertyElement=[__request writeElement:item type:[USAePaySearchParam class] name:@"SearchParam" URI:@"urn:usaepay" parent:__parent skipNullProperty:NO];
        if(propertyElement!=nil)
        {
            [item serialize:propertyElement __request: __request];
        }
    }
    [__request addAttribute:@"type" URI:@"http://www.w3.org/2001/XMLSchema" stringValue:@"xx2:SearchParamArray" element:__parent];
    [__request addAttribute:@"arrayType" URI:@"http://schemas.xmlsoap.org/soap/encoding/" stringValue:@"xx1:SearchParam[]" element:__parent];
...
- (USAePaySearchParam*)objectAtIndex:(NSUInteger)index {
	return [self.items objectAtIndex:index];
}

- (NSArray *)arrayByAddingObject:(USAePaySearchParam*)anObject {
...
- (BOOL)containsObject:(USAePaySearchParam*)anObject {
...
- (NSUInteger)indexOfObject:(USAePaySearchParam*)anObject {
	return [self.items indexOfObject:anObject];
}

- (NSUInteger)indexOfObject:(USAePaySearchParam*)anObject inRange:(NSRange)range {
	return [self.items indexOfObject:anObject inRange:range];
}

- (NSUInteger)indexOfObjectIdenticalTo:(USAePaySearchParam*)anObject {
	return [self.items indexOfObjectIdenticalTo:anObject];
}

- (NSUInteger)indexOfObjectIdenticalTo:(USAePaySearchParam*)anObject inRange:(NSRange)range {
...
- (void)addObject:(USAePaySearchParam*)anObject {
	return [self.items addObject:anObject];
}

- (void)insertObject:(USAePaySearchParam*)anObject atIndex:(NSUInteger)index {
...
- (void)replaceObjectAtIndex:(NSUInteger)index withObject:(USAePaySearchParam*)anObject {
...
- (void)removeObject:(USAePaySearchParam*)anObject inRange:(NSRange)range {
	return [self.items removeObject:anObject inRange:range];
}

- (void)removeObject:(USAePaySearchParam*)anObject {
	return [self.items removeObject:anObject];
}

- (void)removeObjectIdenticalTo:(USAePaySearchParam*)anObject inRange:(NSRange)range {
	return [self.items removeObjectIdenticalTo:anObject inRange:range];
}

- (void)removeObjectIdenticalTo:(USAePaySearchParam*)anObject {
...
+ (id)array {
	return [[ USAePaySearchParamArray alloc] init];
}

+ (id)arrayWithObject:(USAePaySearchParam*)anObject {
	return [[ USAePaySearchParamArray alloc] initWithObjects:anObject, nil];
}

+ (id)arrayWithObjects:(const USAePaySearchParam* *)objects count:(NSUInteger)cnt {
	return [[ USAePaySearchParamArray alloc] initWithObjects:objects count:cnt];
}

+ (id)arrayWithObjects:(USAePaySearchParam*)firstObj, ... {
	USAePaySearchParamArray* a = [ USAePaySearchParamArray array];
	id eachObject;
...
- (id)initWithObjects:(USAePaySearchParam*)firstObj, ... {
	if(self = [self init]) {
		id eachObject;
		va_list argumentList;
		if (firstObj) {
...
+ (id)arrayWithArray:(NSArray *)array {
	return [[ USAePaySearchParamArray alloc] initWithArray:array];
}

+ (id)arrayWithContentsOfFile:(NSString *)path {
	return [[ USAePaySearchParamArray alloc] initWithContentsOfFile:path];
}

+ (id)arrayWithContentsOfURL:(NSURL *)url {
	return [[ USAePaySearchParamArray alloc] initWithContentsOfURL:url];
}

+ (id)arrayWithCapacity:(NSUInteger)numItems {
	return [[ USAePaySearchParamArray alloc] initWithCapacity:numItems];
}

- (id)initWithObjects:(const USAePaySearchParam* *)objects count:(NSUInteger)cnt {
	self = [self init];
	if(self) {
...
- (NSArray *)sortedArrayUsingFunction:(NSInteger (*)(USAePaySearchParam*, USAePaySearchParam*, void *))comparator context:(void *)context {
	return [self.items sortedArrayUsingFunction:comparator context:context];
}

- (NSArray *)sortedArrayUsingFunction:(NSInteger (*)(USAePaySearchParam*, USAePaySearchParam*, void *))comparator context:(void *)context hint:(NSData *)hint {
	return [self.items sortedArrayUsingFunction:comparator context:context hint:hint];
}

- (void)sortUsingFunction:(NSInteger (*)(USAePaySearchParam*, USAePaySearchParam*, void *))compare context:(void *)context {
