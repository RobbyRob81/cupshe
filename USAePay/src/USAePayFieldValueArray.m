...
@implementation USAePayFieldValueArray
...
- (id) initWithXml: (DDXMLElement*) __node __request:(USAePayRequestResultHandler*) __request {
    if(self = [self init])
    {
        ...
        {
            [self.items addObject:(USAePayFieldValue*)[__request createObject:child type:[USAePayFieldValue class]]];
        }
...
{
    for (USAePayFieldValue* item in self.items) {
        DDXMLElement* propertyElement=[__request writeElement:item type:[USAePayFieldValue class] name:@"FieldValue" URI:@"urn:usaepay" parent:__parent skipNullProperty:NO];
        if(propertyElement!=nil)
        {
            [item serialize:propertyElement __request: __request];
        }
    }
    [__request addAttribute:@"type" URI:@"http://www.w3.org/2001/XMLSchema" stringValue:@"xx2:FieldValueArray" element:__parent];
    [__request addAttribute:@"arrayType" URI:@"http://schemas.xmlsoap.org/soap/encoding/" stringValue:@"xx1:FieldValue[]" element:__parent];
...
- (void)removeObjectIdenticalTo:(USAePayFieldValue*)anObject inRange:(NSRange)range {
	return [self.items removeObjectIdenticalTo:anObject inRange:range];
}

- (void)removeObjectIdenticalTo:(USAePayFieldValue*)anObject {
...
+ (id)array {
	return [[ USAePayFieldValueArray alloc] init];
}

+ (id)arrayWithObject:(USAePayFieldValue*)anObject {
	return [[ USAePayFieldValueArray alloc] initWithObjects:anObject, nil];
}

+ (id)arrayWithObjects:(const USAePayFieldValue* *)objects count:(NSUInteger)cnt {
	return [[ USAePayFieldValueArray alloc] initWithObjects:objects count:cnt];
}

+ (id)arrayWithObjects:(USAePayFieldValue*)firstObj, ... {
	USAePayFieldValueArray* a = [ USAePayFieldValueArray array];
	id eachObject;
...
- (id)initWithObjects:(USAePayFieldValue*)firstObj, ... {
	if(self = [self init]) {
		id eachObject;
		va_list argumentList;
		if (firstObj) {
...
+ (id)arrayWithArray:(NSArray *)array {
	return [[ USAePayFieldValueArray alloc] initWithArray:array];
}

+ (id)arrayWithContentsOfFile:(NSString *)path {
	return [[ USAePayFieldValueArray alloc] initWithContentsOfFile:path];
}

+ (id)arrayWithContentsOfURL:(NSURL *)url {
	return [[ USAePayFieldValueArray alloc] initWithContentsOfURL:url];
}

+ (id)arrayWithCapacity:(NSUInteger)numItems {
	return [[ USAePayFieldValueArray alloc] initWithCapacity:numItems];
}
