...
@implementation USAePayCurrencyObjectArray
...
- (id) initWithXml: (DDXMLElement*) __node __request:(USAePayRequestResultHandler*) __request {
    if(self = [self init])
    {
        ...
        {
            [self.items addObject:(USAePayCurrencyObject*)[__request createObject:child type:[USAePayCurrencyObject class]]];
        }
...
{
    for (USAePayCurrencyObject* item in self.items) {
        DDXMLElement* propertyElement=[__request writeElement:item type:[USAePayCurrencyObject class] name:@"CurrencyObject" URI:@"urn:usaepay" parent:__parent skipNullProperty:NO];
        if(propertyElement!=nil)
        {
            [item serialize:propertyElement __request: __request];
        }
    }
    [__request addAttribute:@"type" URI:@"http://www.w3.org/2001/XMLSchema" stringValue:@"xx2:CurrencyObjectArray" element:__parent];
    [__request addAttribute:@"arrayType" URI:@"http://schemas.xmlsoap.org/soap/encoding/" stringValue:@"xx1:CurrencyObject[]" element:__parent];
...
- (void)removeObjectIdenticalTo:(USAePayCurrencyObject*)anObject inRange:(NSRange)range {
	return [self.items removeObjectIdenticalTo:anObject inRange:range];
}

- (void)removeObjectIdenticalTo:(USAePayCurrencyObject*)anObject {
...
+ (id)array {
	return [[ USAePayCurrencyObjectArray alloc] init];
}

+ (id)arrayWithObject:(USAePayCurrencyObject*)anObject {
	return [[ USAePayCurrencyObjectArray alloc] initWithObjects:anObject, nil];
}

+ (id)arrayWithObjects:(const USAePayCurrencyObject* *)objects count:(NSUInteger)cnt {
	return [[ USAePayCurrencyObjectArray alloc] initWithObjects:objects count:cnt];
}

+ (id)arrayWithObjects:(USAePayCurrencyObject*)firstObj, ... {
	USAePayCurrencyObjectArray* a = [ USAePayCurrencyObjectArray array];
	id eachObject;
...
- (id)initWithObjects:(USAePayCurrencyObject*)firstObj, ... {
	if(self = [self init]) {
		id eachObject;
		va_list argumentList;
		if (firstObj) {
...
+ (id)arrayWithArray:(NSArray *)array {
	return [[ USAePayCurrencyObjectArray alloc] initWithArray:array];
}

+ (id)arrayWithContentsOfFile:(NSString *)path {
	return [[ USAePayCurrencyObjectArray alloc] initWithContentsOfFile:path];
}

+ (id)arrayWithContentsOfURL:(NSURL *)url {
	return [[ USAePayCurrencyObjectArray alloc] initWithContentsOfURL:url];
}

+ (id)arrayWithCapacity:(NSUInteger)numItems {
	return [[ USAePayCurrencyObjectArray alloc] initWithCapacity:numItems];
}
